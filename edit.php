...
$query = mysqli_query($conn, "SELECT * FROM pc_list WHERE id = $id");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama_pc) || empty($nama_user) || empty($nomor_asset) || empty($nomor_ip) || empty($status) || empty($tanggal_produksi)) {
        ...
        $check = mysqli_query($conn, "SELECT * FROM pc_list WHERE nomor_asset = '$nomor_asset' AND id != $id");
        if (mysqli_num_rows($check) > 0) {
            $error = "Nomor Asset sudah digunakan!";
        } else {
            $query = "UPDATE pc_list SET 
                      nama_pc = '$nama_pc',
                      nama_user = '$nama_user',
...
<body>
    <div class="container">
        ...
        <div class="header">
            <h1>Edit Data PC</h1>
            <p class="subtitle">Perbarui informasi PC</p>
        </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="nama_pc" placeholder="Contoh: PC-MU-TIK01" required 
                           value="<?= htmlspecialchars($_POST['nama_pc']); ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status" required>
                        ...
                        <option value="Baik" <?= ($_POST['status'] == 'Baik') ? 'selected' : ''; ?>>Baik</option>
                        <option value="Perbaikan" <?= ($_POST['status'] == 'Maintenance') ? 'selected' : ''; ?>>Maintenance</option>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal_produksi" required
                           value="<?= htmlspecialchars($_POST['tanggal_produksi']); ?>">
                </div>
            </div>
